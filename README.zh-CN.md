...
- 确认即将重置时，百分比旁显示 TRAE 小铃铛，前 1 分钟左右摇晃，之后静止保留至公告结束。
  同一条公告的重复检查不会重新播放；
- 确认即将重置时发送 macOS 原生系统通知，同一条公告仅通知一次，重启应用也不会重复。
  首次提示时允许通知即可；拒绝通知权限不影响菜单栏功能；
- 公告提供有效的计划重置时间时，通知会写出完整日期和 24 小时时间，自动换算为 Mac 的本地时区；
  时间缺失或无法解析时保留原来的无时间提醒，不猜测时间；
...
通知由 macOS 在本机发送，不需要推送服务器或 Apple 开发者账号。应用只在本地偏好设置中保存公开公告 ID，
用于避免重复通知。横幅和提示音遵循 macOS 的通知与专注模式设置。如果之前拒绝了权限，可以在
“系统设置 → 通知 → CodexBarSimple → 允许通知”中开启，再重启应用检查当前公告。

通知中的时间取自 `scheduled_reset.scheduled_for`，不会把公告发布时间或概率预测当成重置时间。
显示的是首次发送该公告通知时已知的计划时间；后续时间调整不会重复发送同一公告的通知。
